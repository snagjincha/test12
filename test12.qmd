...
for trace in scatter_data.data:
    trace_id = trace.customdata[0]  # customdata는 각 트레이스가 가지고 있는 고유한 데이터입니다.
    if trace_id not in existing_ids:
        existing_ids.add(trace_id)
        trace.name = f'{trace.name} (pickup/dropoff)'
        fig3.add_trace(trace)
    # 이제 이미 추가된 id에 대해서는 트레이스를 추가하지 않습니다.

# 선 그래프 속성 업데이트
fig3.update_traces(
    line=dict(width=1),  # 선 너비 설정
    opacity=0.8,  # 투명도 설정
    selector=dict(type='scattermapbox', mode='lines')  # 올바른 셀렉터 사용
)
